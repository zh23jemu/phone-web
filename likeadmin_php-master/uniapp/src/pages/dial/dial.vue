	...
	methods: {
	    ...
	    pressKey(key) {
	        ...
	        if (this.audioContext) {
	            let soundSrc = '';
	            switch(key) {
	                case '0': soundSrc = '/static/audio/dtmf-0.mp3'; break;
	                case '1': soundSrc = '/static/audio/dtmf-1.mp3'; break;
	                case '2': soundSrc = '/static/audio/dtmf-2.mp3'; break;
	                case '3': soundSrc = '/static/audio/dtmf-3.mp3'; break;
	                case '4': soundSrc = '/static/audio/dtmf-4.mp3'; break;
	                case '5': soundSrc = '/static/audio/dtmf-5.mp3'; break;
	                case '6': soundSrc = '/static/audio/dtmf-6.mp3'; break;
	                case '7': soundSrc = '/static/audio/dtmf-7.mp3'; break;
	                case '8': soundSrc = '/static/audio/dtmf-8.mp3'; break;
	                case '9': soundSrc = '/static/audio/dtmf-9.mp3'; break;
	                case '*': soundSrc = '/static/audio/dtmf-star.mp3'; break;
	                case '#': soundSrc = '/static/audio/dtmf-hash.mp3'; break;
	            }
	            if (soundSrc) {
                ...
                if (res.statusCode === 200 && res.data && res.data.code === 0) {
                    if (res.data.data) {
                        ...
                        if (incomingCall.status === 'ringing') {
                            ...
                            // 跳转到通话页面，传递来电信息
                            uni.navigateTo({
                                url: `/pages/call/calling?callId=${incomingCall.callId}&number=${encodeURIComponent(incomingCall.number)}&dialerUserId=${incomingCall.dialerUserId}&location=${encodeURIComponent(incomingCall.location || '未知位置')}&markType=${incomingCall.mark_type || ''}`
                            });
                        }
                    }
                } else if (res.statusCode === 200 && res.data && res.data.code === 0 && !res.data.data) {
                     // 没有来电，继续轮询
                     // console.log('No incoming calls.'); // 避免频繁打印日志
                } else {
                    console.error('Failed to check incoming calls:', res.data?.msg);
                    // 可以在这里添加错误处理，例如停止轮询或显示错误消息
                }
            } catch (error) {
                console.error('Error checking incoming calls:', error);
                // 可以在这里添加网络错误处理，例如停止轮询或显示错误消息
            }
        }
        // <=== 结束添加来电轮询方法 ===>
	},
