...
def insert_call_record(call_id, dialer_user_id, backend_user_id, dialed_number, start_time, status, connection, location=None, call_type='normal'):
    ...
    cursor = connection.cursor()
    try:
        sql = "INSERT INTO `la_call_records` (`call_id`, `dialer_user_id`, `backend_user_id`, `dialed_number`, `start_time`, `status`, `location`, `call_type`, `created_at`, `updated_at`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        created_at = updated_at = int(time.time())
        cursor.execute(sql, (call_id, dialer_user_id, backend_user_id, dialed_number, start_time, status, location, call_type, created_at, updated_at))
        connection.commit()
        print(f"Inserted new call record with ID: {call_id}")
        return True
    except mysql.connector.Error as err:
            ...
            # 处理挂断消息
                ...
                if call_to_hangup and (call_to_hangup['dialer_user_id'] == current_user_id or call_to_hangup['backend_user_id'] == current_user_id):
                    ...
                    if update_call_status(call_id, end_status, conn):
                        ...
                        self.end_headers()
                        response_data = {'code': 0, 'msg': 'Call ended successfully'}
                    ...
                    print(f"Invalid hangupCall request from user {current_user_id} for call {call_id}")
                    await websocket.send(json.dumps({'type': 'error', 'message': f"Invalid request for call {call_id}"}))

            # 处理铃声控制消息
            elif data.get('type') == 'toggleRingtone':
                is_playing = data.get('isPlaying', False)
                print(f"User {current_user_id} toggled ringtone: {is_playing}")
                ...
                for user_id, ws in connected_users.items():
                    try:
                        ...
                        print(f"Error sending ringtone status to user {user_id}: {send_err}")
...
async def check_heartbeat(websocket, last_ping_time):
    ...
    try:
        while True:
            ...
            if time.time() - last_ping_time > 60:  # 如果超过60秒没有收到心跳
...
async def websocket_handler(websocket):
    ...
    try:
        ...
        if disconnected_user_id:
            ...
            if conn:
                try:
                    # 查找该用户作为拨号方或被叫方参与的未结束的呼叫
                    calls_involved = [
                        call_id for call_id, call_info in list(get_user_active_calls(disconnected_user_id, conn))
                        if (call_info.get('dialer_user_id') == disconnected_user_id or call_info.get('backend_user_id') == disconnected_user_id)
                           and call_info.get('status') != 'ended'
                    ]

                    for call_id in calls_involved:
                        call = get_active_call(call_id, conn)
                        if call:
                            # 检查断开连接的用户是否是拨号方或被叫方
                            if disconnected_user_id not in [call.get('dialer_user_id'), call.get('backend_user_id')]:
                                print(f"User {disconnected_user_id} is not involved in call {call_id}, ignoring disconnection")
                                continue

                            end_time = int(time.time())
                            # 根据断开时的状态和用户角色决定结束状态
                            if call['status'] == 'ringing':
                                # 如果是响铃状态，根据断开的是拨号方还是被叫方决定状态
                                if call['dialer_user_id'] == disconnected_user_id:
                                    end_status = 'canceled'  # 拨号方挂断
                                else:
                                    end_status = 'no_answer'  # 被叫方未接听
                            else:
                                end_status = 'completed'  # 通话中挂断

                            # 更新数据库中的呼叫状态和结束时间/时长
                            update_call_status(call_id, end_status, conn)
                            update_call_end_time_and_duration(call_id, call.get('start_time'), end_time, conn)

                            # 通知另一方用户（如果在线且有 WS 连接）
                            other_user_id = None
                            if call.get('dialer_user_id') == disconnected_user_id:
                                other_user_id = call.get('backend_user_id')
                            elif call.get('backend_user_id') == disconnected_user_id:
                                other_user_id = call.get('dialer_user_id')
                            ...
                            if other_user_id and other_user_id in connected_users:
                                ...
                                if other_user_ws:
                                    try:
                                        ...
                                        await other_user_ws.send(json.dumps(call_ended_message))
                                        print(f"Sent callEnded to user {other_user_id}.")
                                    except Exception as send_err:
                                        print(f"Error sending callEnded to user {other_user_id} on disconnect: {send_err}")

                            # 从内存中移除呼叫状态
                            get_active_call(call_id, conn)
                            print(f"Call {call_id} ended with status {end_status} due to user {disconnected_user_id} disconnection.")

                except Exception as db_cleanup_err:
                    print(f"Database error during client {disconnected_user_id} disconnection cleanup: {db_cleanup_err}")
                finally:
                    close_db_connection(conn)
    ...
    def do_POST(self):
        ...
        if parsed_url.path == '/api/dial':
            ...
            try:
                ...
                if not call_id:
                    ...
                    self.send_header('Content-type', 'application/json')
                    self.end_headers()
                    response_data = {'code': 1, 'msg': 'Missing callId in request'}
                    self.wfile.write(json.dumps(response_data).encode('utf-8'))
                    return
                ...
                try:
                    ...
                    if not call:
                        ...
                        self.send_header('Content-type', 'application/json')
                        self.end_headers()
                        response_data = {'code': 1, 'msg': 'Call not found'}
                        self.wfile.write(json.dumps(response_data).encode('utf-8'))
                        return

                    end_time = int(time.time())

                    # 检查目标用户的WebSocket连接状态
                    target_ws = connected_users.get(call['backend_user_id'])
                    is_connected = False
                    if target_ws:
                        try:
                            is_connected = True
                    ...
                    if update_call_status(call_id, end_status, conn):
                        print(f"Successfully updated status for call {call_id} to {end_status}.")
                        # 更新结束时间和时长
                        if update_call_end_time_and_duration(call_id, call['start_time'], end_time, conn):
                            print(f"Successfully updated end time and duration for call {call_id}.")
                        else:
                            print(f"Failed to update end time and duration for call {call_id}.")
                        ...
                        print(f"Failed to update DB status for call {call_id} on hangup.")
                        self.send_error_response(f'Failed to update call status for {call_id}')

                except Exception as e:
                    print(f"Error processing hangup request: {e}")
                    self.send_response(HTTPStatus.INTERNAL_SERVER_ERROR)
                    self.send_header('Content-type', 'application/json')
                    self.end_headers()
            ...
            try:
                ...
                print(f"Attempting to delete call record with ID: {call_ids}")
                ...
                # 删除通话记录
                sql = f"DELETE FROM `la_call_records` WHERE `call_id` IN ({placeholders})"
                cursor.execute(sql, tuple(call_ids))
                conn.commit()

                print(f"Successfully deleted call records with IDs: {call_ids}")
                self.send_response(HTTPStatus.OK)
                self.send_header('Content-type', 'application/json')
                self.send_header('Access-Control-Allow-Origin', '*')
                self.end_headers()
                response_data = {'code': 0, 'msg': 'Call records deleted successfully'}
                self.wfile.write(json.dumps(response_data).encode('utf-8'))
                ...
                self.send_header('Access-Control-Allow-Origin', '*')
                self.end_headers()
                response_data = {'code': 1, 'msg': 'Failed to delete call records', 'error': str(err)}
                self.wfile.write(json.dumps(response_data).encode('utf-8'))
                ...
                self.send_header('Access-Control-Allow-Origin', '*')
                self.end_headers()
                response_data = {'code': 1, 'msg': 'Internal server error', 'error': str(err)}
                self.wfile.write(json.dumps(response_data).encode('utf-8'))
            finally:
                if 'cursor' in locals():
                    cursor.close()
        ...
        # 发送消息
            ...
            try:
                ...
                # Validate ismy is a boolean
                if not isinstance(ismy, bool):
                    self.send_error_response('Invalid ismy parameter: must be boolean')
                    return
                ...
                try:
                    ...
                    """
                    cursor.execute(insert_message_sql, (sender_id, receiver_phone, content, ismy, current_time, current_time, current_time))
                    message_id = cursor.lastrowid

                    # 更新或创建发送者的会话
                    update_sender_conversation_sql = """
                        INSERT INTO `la_conversations` 
                        (user_id, phone, last_message, last_message_time, unread_count, created_at, updated_at)
                    ...
                    cursor.execute(update_sender_conversation_sql,
                                 (sender_id, receiver_phone, content, current_time, current_time, current_time))
                    ...
                    cursor.execute(update_receiver_conversation_sql,
                                 (receiver_phone, sender_id, content, current_time, current_time, current_time))
        ...
        # 删除单个会话
            ...
            return # Add return after /api/delete-conversation processing
        ...
        # 批量删除会话
            ...
            return # Add return after /api/delete-conversations processing
        ...
        # 获取联系人信息
            ...
            try:
                ...
                try:
                    ...
                    # 查询联系人表
                    sql = "SELECT id, user_id, phone, name FROM `la_contacts` WHERE `user_id` = %s AND `phone` = %s LIMIT 1"
        ...
        if parsed_url.path == '/api/call-records':
            ...
            if user_id_str:
                try:
                    ...
                    pass # Invalid userId format, will fetch no records or handle error
            ...
            try:
                ...
                print("Invalid page or pageSize parameter. Using defaults.")
                page = 1
                page_size = 20
            ...
            # ===> 解析可选的 number 参数 <===
            filter_number = query_params.get('number', [None])[0]
            # <=== 结束解析可选的 number 参数 ===>
            ...
            try:
                cursor = conn.cursor(dictionary=True)
               # 在 cursor.execute 之前初始化 rows
                rows = []

                # 构建 SQL 查询
                sql = ""
                sql += " SELECT cr.*, lm.mark_type,"
                sql += "        CASE"
                sql += "            WHEN c.name IS NOT NULL THEN c.name"
                sql += "            ELSE cr.dialed_number"
                sql += "        END as display_name,"
                sql += "        c.name as contact_name"
                sql += " FROM `la_call_records` cr"
                sql += " LEFT JOIN `la_call_mark` lm ON cr.dialed_number = lm.dialed_number"
                # Include JOIN to la_contacts only if target_user_id is provided
                if target_user_id is not None:
                     sql += " LEFT JOIN `la_contacts` c ON cr.dialed_number = c.phone AND c.user_id = %s"
                else:
                     # If no user ID, still need 'c' alias in SELECT for display_name/contact_name
                     # but the JOIN condition related to user_id is omitted.
                     # This might require adjusting the SELECT if c is not joined. A simpler fix is to always join but make the ON condition optional.
                     # Let's stick to the original JOIN structure but ensure the user_id parameter is handled correctly.
                     # Reverting to always joining, as the original approach seemed intended.
                     sql = ""
                     sql += " SELECT cr.*, lm.mark_type,"
                     sql += "        CASE"
                     sql += "            WHEN c.name IS NOT NULL THEN c.name"
                ...
                sql_count = '''
                    ...
                    LEFT JOIN `la_contacts` c ON cr.dialed_number = c.phone AND c.user_id = %s -- Include JOIN condition for count
                '''
                conditions = []
                params = [] # Start with an empty list and add parameters based on conditions
                count_params = []

                 # Add parameter for c.user_id JOIN condition (if user ID is present) - this param needs to be first for the main query
                if target_user_id is not None:
                     params.append(target_user_id)
                     count_params.append(target_user_id) # Add to count params as well
                else:
                    # If no target_user_id, the c.user_id join condition is effectively bypassed,
                    # but the SQL still expects a parameter for %s in the JOIN clause.
                    # We can pass None or a default if the database/connector handles it,
                    # or modify the JOIN dynamically if target_user_id is None.
                    # Let's simplify and assume target_user_id is always needed for JOIN, and if not provided, it's an error for this endpoint (handled above).
                    # Or, if number is provided without user, we still need a placeholder for the JOIN %s.
                    # A safer approach might be to dynamically build the JOIN clause based on target_user_id.
                    # Reverting to requiring userId for the JOIN condition parameter.
                    # If target_user_id is None, the initial check for missing userId will handle it.
                    pass # param is already added if target_user_id is not None



                # Add user filtering condition
                if target_user_id is not None:
                    conditions.append('(cr.dialer_user_id = %s OR cr.backend_user_id = %s)')
                    params.extend([target_user_id, target_user_id])
                    count_params.extend([target_user_id, target_user_id])


                # ===> Add number filtering condition <===
                if filter_number:
                    conditions.append('cr.dialed_number = %s')
                    params.append(filter_number)
                    count_params.append(filter_number)
                # <=== End Add number filtering condition ===>
                ...
                # <=== End Apply LIMIT and OFFSET for pagination ===>
                ...
                end_time = time.time()
                print(f"Query for /api/call-records took {end_time - start_time:.4f} seconds.")

                # ===> Get total count based on filtering conditions <===
                print("Executing count SQL:", sql_count)
                print("With count parameters:", count_params)

                start_time_count = time.time()
                cursor.execute(sql_count, count_params)
                total_count = cursor.fetchone()['COUNT(*)']
                end_time_count = time.time()
                print(f"Count query for /api/call-records took {end_time_count - start_time_count:.4f} seconds.")
                # <=== End Get total count based on filtering conditions ===>
                ...
                return self.wfile.write(json.dumps(response_data).encode('utf-8'))
                # <=== 结束返回分页和总数信息 ===>


            except mysql.connector.Error as err:
                print(f"Error fetching call records: {err}")
                self.send_response(HTTPStatus.INTERNAL_SERVER_ERROR)
                self.send_header('Content-type', 'application/json')
                self.send_header('Access-Control-Allow-Origin', '*') # Allow CORS
                self.end_headers()
                response_data = {'code': 1, 'msg': 'Failed to fetch call records', 'error': str(err)}
                self.wfile.write(json.dumps(response_data).encode('utf-8'))
            finally:
                cursor.close()
                close_db_connection(conn)
        ...
        # 获取会话列表
            ...
            try:
                ...
                sql = """
                    ...
                    LEFT JOIN `la_contacts` ct ON c.phone = ct.phone AND ct.user_id = %s
                    WHERE c.user_id = %s
                    ORDER BY c.last_message_time DESC
                """
                print("Executing SQL for /api/conversations:", sql)
                print("With parameters:", (user_id, user_id))
...
async def main():
    ...
    try:
        ...
        # 如果 WS 启动失败，可以选择退出或只启动 HTTP

    # ===> 启动 HTTP 服务器 (在单独的线程中运行，使用不同的端口，例如 9097) <===
    http_port = 9097 # 使用不同的端口
    try:
        # 使用 socketserver.TCPServer 来运行同步的 http.server
        httpd = socketserver.TCPServer(("0.0.0.0", http_port), CallRecordsHTTPHandler)
        print(f"HTTP server listening on port {http_port}")
        # 在单独的线程中运行 HTTP 服务器，以免阻塞 asyncio 事件循环
        http_thread = threading.Thread(target=httpd.serve_forever)
        http_thread.daemon = True # 设置为守护线程，主线程退出时自动退出
        http_thread.start()
    except Exception as http_err:
         print(f"Failed to start HTTP server on port {http_port}: {http_err}")
         # 如果 HTTP 服务器启动失败，决定是否继续运行 WS 服务器
    ...
    if 'ws_server' in locals():
        ...
        print("WebSocket server did not start.")
        # 如果 WS 没有启动，主线程可能会立即退出，如果 HTTP 在守护线程中运行
