		...
		methods: {
		  decodeUtf8Bytes(bytes) {
			...
			for (let i = 0; i < bytes.length; i += 1) {
				const byte1 = bytes[i];
				if (byte1 < 0x80) { result += String.fromCharCode(byte1); continue; }
		  ...
		  fixGarbledText(text) {
			...
			try {
				const bytes = Array.from(text).map(char => {
					...
					if (CP1252_BYTE_MAP[code] !== undefined) return CP1252_BYTE_MAP[code];
		  ...
		  async checkContactAndDisplay(number, defaultLocation) {
		    ...
		    try {
		        ...
		        if (res.statusCode === 200 && res.data && res.data.code === 0 && res.data.data) {
		            ...
		            this.number = fixGarbledText(res.data.data.name);  // Display name
				...
				try {
					...
					if (response.data.code === 0) {
						const records = response.data.data.list.map(record => ({
							...
							displayName: fixGarbledText(record.contact_name) || record.dialed_number,
