<template>
  <view class="chat-container">
    ...
    <view class="chat-header">
      ...
      <image class="chat-avatar" src="/static/icons/avatar1.png" mode="aspectFill"></image>
      <view class="chat-phone">{{ displayName || phone }}</view>
      ...
      try {
        ...
        if (response.data.code === 0) {
          ...
          const formattedMessages = list.map(msg => ({
            ...
            ismy: msg.ismy === 1 // 显式判断，如果后端返回1，则认为是自己的消息
          })).reverse();

          console.log('Formatted messages with ismy:', formattedMessages);
    ...
    async checkContactAndDisplay(number) {
      ...
      try {
          ...
          if (res.statusCode === 200 && res.data && res.data.code === 0 && res.data.data) {
              ...
              console.log('checkContactAndDisplay: Contact not found or API issue. Keeping initial displayName.');
          }
      } catch (error) {
          console.error('checkContactAndDisplay: Failed to check contact API:', error);
          // On API failure - keep initial displayName
      }
    }
  }
};
